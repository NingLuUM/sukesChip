...
void rcvSysMsgHandler(RCVsys_t *RCV, FMSG_t *msg, int *runner){
    ...
    switch(msg->u32[0]){
        case(CASE_RCV_SET_RECLEN):{
            RCV->setRecLen(RCV,msg->u32[1]);
            if( msg->u32[2] ){
                printf("hello from here\n");
				setupInternalStorage(RCV);
                printf("hello from here\n");
			}
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_RCV_STATE_RESET):{
            RCV->setStateReset(RCV,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_ADC_SET_UNSIGNED):{
            RCV->ADC->setUnsignedInt(RCV->ADC, msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_ADC_SET_LOW_NOISE_MODE):{
            RCV->ADC->setLowNoiseMode(RCV->ADC, msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_ADC_TOGGLE_CHANNEL_POWER):{
            RCV->ADC->toggleChannelPower(RCV->ADC,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_ADC_SET_FILTER_BW):{
            RCV->ADC->setFilterBW(RCV->ADC,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_ADC_SET_INTERNAL_AC_COUPLING):{
            RCV->ADC->setInternalAcCoupling(RCV->ADC,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_RCV_SET_CLOCK_DIVISOR):{
            RCV->setClkDivisor(RCV,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_RCV_SET_SAMPLING_MODE):{
            RCV->setSamplingMode(RCV,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_RCV_SET_COMPRESSOR_MODE):{
            RCV->setCompressorMode(RCV,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
        ...
        case(CASE_RCV_INTERRUPT_THYSELF):{
            RCV->refVals.pioSettings.interrupt_thyself = msg->u32[1];
        ...
        case(CASE_ADC_SET_POWER_ON_OFF):{
            RCV->ADC->setPowerState(RCV->ADC,msg->u32[1]);
        ...
        case(CASE_ADC_DISABLE_CLAMP):{
            RCV->ADC->disableClamp(RCV->ADC,msg->u32[1]);
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
		...
		case(CASE_ADC_SET_FCLOCK_DELAY):{
			RCV->refVals.pioSettings.fclk_delay = msg->u32[1];
			DREF32(RCV->pioSettings) = RCV->refVals.pioSettings.all;
            if ( send(RCV->comm_sock->fd,&(RCV->npulses),sizeof(uint32_t),MSG_CONFIRM) && RCV->printMsgs ){
